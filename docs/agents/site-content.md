...
- Static site assets (images and other files served as-is by site path) go in
  ...
  generated by `bun run hero` (a real, isolated herdr capture; see
...
| ------------------- | --------------------------------------------------------------- |
| `bun run docs:dev`  | Starts the local dev server with live reload                    |
| `bun run docs:build`| Builds the site; a broken internal link fails the build with a readable error naming the page and the link |
| `bun run docs:preview` | Serves the built site locally before it is published        |

Run `bun run docs:build` before pushing. CI runs the same build on every pull
