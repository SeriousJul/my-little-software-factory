...
Status: accepted
Date: 2026-09-13
Superseded in part by ADR 0019: the decisions that one expanded section owns both list and detail, and that `t` and `v` switch sections, no longer hold. The Main surface, the single Message line, the single Action bar, the shared control catalogue, and the Consultation header's attention facts survive.
