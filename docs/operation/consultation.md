...
- `j` and `k` cross into the Consultation section from the last row of the
	...
	it never changes what the operator is looking at in herdr (ADR 0061). A
	`missing`, a `failed`, a `queued`, or an `unscheduled` Consultation closes
	without a dialog: these hold nothing live to stop, and the `queued` one's
	Work queue item leaves with the record. A `closing` one opens the Retry and
	Force-close recovery panel instead, and
