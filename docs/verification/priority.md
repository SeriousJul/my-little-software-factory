...
| The fixing ticket feeds the same rule: a pull request with no rank of its own takes the highest effective priority of the tickets it fixes, the closing references and the fixing ticket feed one rule, an override set on the fixing ticket travels through, the pull request's own Priority override still wins, a covered ticket hidden from the list still supplies its rank, and a ticket that left the source leaves the branch-linked pull request unranked | `test/fixing-pull-request.test.ts` | Passed |
...
- Measured on 2026-09-21 on Bun 1.4.2: `bun run lint` passed, `bun run typecheck` passed, `bun test` ran 1775 tests across 75 files with 1762 pass, 13 skip, and 0 fail. The skip set is the recorded one above; no skip is new or different.
