...
| The first run says it is downloading before it asks the network for anything, and a cached run says nothing | `test/installer.test.ts`, with the fetch and the writer faked; and the packed `./node_modules/.bin/factory` printed the note line before its incomplete-release line, exit 1, re-run against the real GitHub release on 2026-09-24 | Passed |
