...
| The continuation walk's guard list is measured guard by guard in auto mode, each with the route isolated from the cycle's other adds: the position open, offering the outcome's task, actionable, with no unfinished attempt, past the Same-type hold, and under its handoff limit (ADR 0051, story 43). Awaiting the position's own settled turn closes it first, and the same cycle's route then stands on the freed position - the order, not a hold, is what that walk measures | `test/observation.test.ts` | Passed |
| An item the pickup dropped is reconsidered by the top-up every cycle the queue is empty, the restart included: the ask's start report clears the episode mark on every exit that ends the item without a start, and a restart that started keeps it until the ticket leaves in-flight (ADR 0051, story 24) | `test/observation.test.ts` | Passed |
