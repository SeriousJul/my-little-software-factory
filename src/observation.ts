 ...
 * Each cycle:
 *
 * 1. An in-flight ticket whose pane has an agent `working` runs; one whose
 *    agent is `done` settles: the pane's last output is captured into a
 *    completion trace and the ticket rests in awaiting.
 * 2. A pane herdr no longer lists is `missing`.
 * 3. An awaiting ticket resolves by rule: an auto-close type closes at any
 *    time; a fully determined workflow route (one edge, one target) hands
 *    off while auto-handoff is on; anything else waits for a human decision.
 * 4. With auto-handoff on and room under the parallel limit, the first
 *    actionable open ticket is handed off with the configured defaults.
 *
...
export interface AgentReader {
	...
	/** The pane's last output, ANSI stripped. Null when it cannot be read. */
...
export class HerdrAgentReader implements AgentReader {
	...
	async readPane(paneId: string): Promise<string | null> {
		const result = await this.runner.run("herdr", ["agent", "read", paneId]);
		if (result.code !== 0) return null;
		try {
			const data = JSON.parse(result.stdout) as { result?: { output?: unknown } };
			if (typeof data.result?.output === "string") return stripAnsi(data.result.output);
		} catch {
 ...
 *
 * - `close`: the factory closes the ticket now (auto-close type at any
 *   time; auto-handoff on with no usable route).
 * - `route`: a workflow handoff follows the ticket's one-and-only edge.
 * - `wait`: a human decides; the ticket rests in awaiting.
 */
...
interface ObservationOptions {
	...
	dispatch: (intent: HandoffIntent) => Promise<void>;
	now: () => number;
...
export class ObservationCoordinator {
	...
	private readonly dispatch: (intent: HandoffIntent) => Promise<void>;
	...
	private async cycle(): Promise<void> {
		...
		for (const ticket of inFlight) {
			...
			if (agent === undefined) {
				if (autoOn) {
					changed = (await this.handleMissing(ticket, inFlight.length)) || changed;
					if (this.stopped) return;
				}
				continue;
			}
		...
		for (const ticket of this.state.ticketsByState(["awaiting"])) {
			if (this.handleAwaiting(ticket, autoOn)) changed = true;
		}

		// A fresh count: tickets this cycle settled are no longer in flight.
		if (autoOn) {
			const inFlightNow = this.state.ticketsByState(["handed-off", "running"]).length;
			changed = this.dispatchOpen(inFlightNow) || changed;
		}
	...
	private async settle(
		...
		const message = (await this.herdr.readPane(agent.paneId)) ?? "";
	 ...
	 * abandoned instead. Manual mode leaves it for the operator's panel.
	 * A ticket already restarted this episode is not restarted again until
	 * the episode ends.
	 */
	private async handleMissing(ticket: HandoffTicket, inFlightCount: number): Promise<boolean> {
		const limit = this.config.maxHandoffsPerTicket;
		const handoffCount = this.state.handoffCount(ticket.ticketIdentity);
		if (handoffCount >= limit) {
			...
			this.onStatus(
				"warning",
				`ticket ${ticket.ticketIdentity} abandoned: its handoff limit is ${limit}`,
			);
			return true;
		}
		// The missing agent is gone, so it does not hold a parallel slot.
		if (inFlightCount - 1 >= this.config.maxParallelAgents && this.config.maxParallelAgents > 0) {
			return false;
		}
	...
	/**
	 * Resolve an awaiting ticket by rule. Returns whether the cycle changed
	 * factory state.
	 */
	private handleAwaiting(
		ticket: {
			ticketIdentity: string;
			handoffAttemptId: string;
			taskType: string;
		...
		const decision = this.decideAwaiting(ticket.taskType, autoOn);
		...
		if (decision === "close") {
			...
			this.onStatus("info", `ticket ${ticket.ticketIdentity} auto-closed`);
			return true;
		}
		...
		const previousMessage = this.state.lastCompletion(ticket.ticketIdentity)?.message ?? "";
		const choice: HandoffChoice = {
			agentType: edge.agent ?? this.config.defaultAgent,
			environment: edge.environment ?? this.config.defaultEnvironment,
			taskType: edge.to[0],
			model: "",
			thinking: "",
		};
		...
		this.onStatus("info", `ticket ${ticket.ticketIdentity} routed to ${edge.to[0]}`);
		return true;
	}

	/**
	 * The awaiting rule: auto-close types close at any time; a fully
	 * determined route (one edge, one target that is not itself an auto-close
	 * type) routes while auto-handoff is on; a full parallel limit waits
	 * rather than routing; everything else waits for a human.
	 */
	decideAwaiting(taskType: string, autoOn: boolean): AwaitingDecision {
		const task = this.config.taskTypes[taskType];
		if (task?.autoClose === true) return "close";
		const edge = this.singleEdge(taskType);
		if (edge !== undefined && edge.to.length === 1) {
			const target = this.config.taskTypes[edge.to[0]];
			if (target?.autoClose !== true) {
				if (!autoOn) return "wait";
				const inFlight = this.state.ticketsByState(["handed-off", "running"]).length;
				if (this.config.maxParallelAgents > 0 && inFlight >= this.config.maxParallelAgents)
					return "wait";
				return "route";
			}
		}
		if (autoOn) return "close";
		return "wait";
	}
	...
	/** With auto-handoff on and room under the parallel limit, dispatch. */
	private dispatchOpen(inFlightCount: number): boolean {
		if (this.config.maxParallelAgents > 0 && inFlightCount >= this.config.maxParallelAgents)
			return false;
		const tickets = this.state.visibleTickets(this.config.taskRules, this.config.defaultTaskType);
		const next = tickets.find((ticket) => ticket.state === "open" && ticket.actionable);
		if (next === undefined) return false;
		const choice: HandoffChoice = {
			agentType: this.config.defaultAgent,
			environment: this.config.defaultEnvironment,
			taskType: next.suggestedTaskType,
			model: "",
			thinking: "",
		};
		void this.dispatch({
			origin: "open",
			ticketIdentity: next.identity,
			choice,
			previousMessage: "",
		});
		this.onStatus("info", `auto-handoff: handing off ticket ${next.identity}`);
		return true;
	}
}

/**
 * The marker badges a ticket row can hold, derived on the UI side from the
 * state and the last observation: a pane herdr no longer lists is
 * `missing`; an agent reporting blocked is `blocked`.
 */
export type TicketMarker = "blocked" | "missing";

/** In-flight states the observation loop tracks. */
export const IN_FLIGHT_STATES: readonly TicketState[] = ["handed-off", "running"];
