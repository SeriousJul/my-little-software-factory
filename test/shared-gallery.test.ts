...
describe("the shared control gallery", () => {
	...
	test("the priority example shows the rank badge, each selector state, and the Consultation reason", async () => {
		...
		// The Priority fact line, in each of the states the selector steps to.
		expect(text).toContain("Priority: critical (set by you)");
		expect(text).toContain("Priority: off (set by you)");
		expect(text).toContain("Priority: none");
		// The written guide line under it: the keys, and what a step does.
		expect(text).toContain(
		...
		expect(text).toContain("a Consultation has no priority: the bump applies to tickets only");
