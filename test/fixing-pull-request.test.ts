...
describe("the rank through the fixing ticket", () => {
	test("a pull request with no rank of its own takes the highest effective priority of the tickets it fixes", () => {
		const state = openFactoryState(statePath());
		state.initializeSources([security, pulls]);
		state.applyFetch(security, success([securityTicket({ labels: ["critical"] })]));
		state.applyFetch(pulls, success([securityPullTicket()]));
		// The security item is covered and hidden, and still supplies the
		// rank the pull request inherits from it.
		const visible = state.visibleTickets([], "implement", RANKS);
		const [pull] = visible.filter((ticket) => ticket.identity === securityPullIdentity);
	...
	test("the closing references and the fixing ticket feed one rule", () => {
		const state = openFactoryState(statePath());
		state.initializeSources([issues, security, pulls]);
		state.applyFetch(issues, success([issueTicket(issueIdentity, { labels: ["high"] })]));
		state.applyFetch(security, success([securityTicket({ labels: ["critical"] })]));
		// The pull request fixes both: the issue through its closing
		// reference, the security item through its branch.
		state.applyFetch(
			pulls,
			success([
				pullTicket(
					securityPullIdentity,
					97,
					withIssueReferences(withHeadBranch({ draft: "false" }, securityBranch), [
						ref(issueIdentity, 5),
					]),
				),
			]),
		...
		if (pull === undefined) throw new Error("missing pull request ticket");
		// The best effective priority across both links wins.
		expect(pull.priority).toEqual({
			rank: 0,
			label: "critical",
			source: "inherited",
	...
	test("a tie at the best rank names the lowest ticket number", () => {
		const state = openFactoryState(statePath());
		state.initializeSources([issues, security, pulls]);
		state.applyFetch(issues, success([issueTicket(issueIdentity, { labels: ["critical"] })]));
		state.applyFetch(security, success([securityTicket({ labels: ["critical"] })]));
		state.applyFetch(
			pulls,
			success([
	...
	test("the pull request's own Priority override still wins", () => {
		const state = openFactoryState(statePath());
		state.initializeSources([security, pulls]);
		state.applyFetch(security, success([securityTicket({ labels: ["critical"] })]));
		state.applyFetch(pulls, success([securityPullTicket()]));
		state.setPriorityOverride(securityPullIdentity, "low");
		const [pull] = state
			.visibleTickets([], "implement", RANKS)
			.filter((ticket) => ticket.identity === securityPullIdentity);
		if (pull === undefined) throw new Error("missing pull request ticket");
	...
	test("an override set on the fixing ticket travels through inheritance", () => {
		const state = openFactoryState(statePath());
		state.initializeSources([security, pulls]);
		state.applyFetch(security, success([securityTicket({ labels: ["low"] })]));
		state.applyFetch(pulls, success([securityPullTicket()]));
		state.setPriorityOverride(securityIdentity, "critical");
		const [pull] = state
			.visibleTickets([], "implement", RANKS)
			.filter((ticket) => ticket.identity === securityPullIdentity);
		if (pull === undefined) throw new Error("missing pull request ticket");
	...
	test("a ticket that left the source leaves the pull request unranked", () => {
		const state = openFactoryState(statePath());
		state.initializeSources([security, pulls]);
		state.applyFetch(security, success([securityTicket({ labels: ["critical"] })]));
		state.applyFetch(pulls, success([securityPullTicket()]));
		// The security source's next refresh no longer lists the alert: the
		// branch link carries no stored fact, so the rank it supplied leaves.
		state.applyFetch(security, success([]));
		const [pull] = state
			.visibleTickets([], "implement", RANKS)
			.filter((ticket) => ticket.identity === securityPullIdentity);
		if (pull === undefined) throw new Error("missing pull request ticket");
		expect(pull.priority.rank).toBeNull();
		state.close();
	});

	test("a pull request with a rank of its own does not inherit", () => {
		const state = openFactoryState(statePath());
		state.initializeSources([security, pulls]);
		state.applyFetch(security, success([securityTicket({ labels: ["critical"] })]));
		state.applyFetch(pulls, success([securityPullTicket({ labels: ["high"] })]));
		const [pull] = state
			.visibleTickets([], "implement", RANKS)
			.filter((ticket) => ticket.identity === securityPullIdentity);
		if (pull === undefined) throw new Error("missing pull request ticket");
